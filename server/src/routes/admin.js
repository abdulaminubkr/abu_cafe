...
router.post('/customers', staff, async (req, res) => {
  ...
  const existing = await queryOne('SELECT id FROM customers WHERE lower(email) = $1', [String(email).toLowerCase()]);
  ...
  const customer = await queryOne(
    `INSERT INTO customers (full_name, email, phone, address, password_hash)
     VALUES ($1,$2,$3,$4,$5) RETURNING id, full_name, email, phone, address, status, created_at`,
    [full_name, String(email).toLowerCase(), phone || null, address || null, hashPassword(password || 'Customer@123')]
  );
  await logAction(req.user, 'Registered customer', full_name);
