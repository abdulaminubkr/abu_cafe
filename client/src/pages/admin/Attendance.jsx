      ...
      <div className="card">
        ...
        <form onSubmit={handleSave}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Intern</th>{STATUSES.map((s) => <th key={s} style={{ textAlign: 'center' }}>{s}</th>)}</tr></thead>
              <tbody>
                {interns.map((i) => (
                  <tr key={i.id}>
                    <td>{i.full_name}</td>
                ...
                {interns.length === 0 && <tr><td colSpan={5} className="text-center text-muted" style={{ padding: '1.5rem' }}>No active interns.</td></tr>}
              </tbody>
            </table>
          </div>
