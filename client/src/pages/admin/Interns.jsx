...
export default function Interns() {
  ...
  return (
    <DashboardLayout title="Intern Management">
      <div className="flex-between mb-3" style={{ flexWrap: 'wrap', gap: '.6rem' }}>
        ...
        <Link to="/admin/interns/new" className="btn btn-primary"><i className="fa-solid fa-user-plus" /> Register Intern</Link>
      </div>

      <div className="card">
        <div className="table-wrap">
          <table>
            <thead><tr><th></th><th>Name</th><th>Institution</th><th>Matric No</th><th>Phone</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {interns.map((i) => (
                <tr key={i.id}>
                  <td>
                  ...
                  <td>{i.phone}</td>
                  <td>
                    <span className={`badge ${i.status === 'active' ? 'badge-success' : i.status === 'completed' ? 'badge-secondary' : 'badge-danger'}`}>
                      {i.status}
                    </span>
                  </td>
                  <td>
              ...
              {interns.length === 0 && <tr><td colSpan={7} className="text-center text-muted" style={{ padding: '2rem' }}>No interns found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
